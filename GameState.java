...
public class GameState extends State{
	...
	private Chronometer enemySpawner;
	...
	public GameState()
	{
		...
		enemySpawner = new Chronometer();
		enemySpawner.run(Constants.ENEMY_SPAWN_RATE);
	}
	
	
	public void addScore(int value, Vector2D position) {
		score += value;
		messages.add(new Message(position, true, "+"+value+" PUNTOS", Color.BLACK, false, Assets.fontMed));
	}
	
	public void divideCloud(Cloud cloud){
		
		Size size = cloud.getSize();
		...
		for(int i = 0; i < size.quantity; i++){
			movingObjects.add(new Cloud(
					cloud.getPosition(),
					new Vector2D(0, 1).setDirection(Math.random()*Math.PI*2),
					Constants.CLOUD_VEL*Math.random() + 1,
	...
	private void startWave(){
		...
		messages.add(new Message(new Vector2D(Constants.WIDTH/2, Constants.HEIGHT/2), false,
				"PARTIDA "+waves, Color.BLACK, true, Assets.fontBig));
	...
	public void update()
	{
		...
		for(int i = 0; i < movingObjects.size(); i++)
			if(movingObjects.get(i) instanceof Cloud)
				return;
	...
	//se encarga de actualizar todos los obejtos que se muevan en el juego
