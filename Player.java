...
public class Player extends MovingObject{
	...
	private Vector2D heading;	//siempre va ser unitario, direccion
	private Vector2D acceleration;

	private boolean accelerating = false;
	private Chronometer fireRate;
	
	//reaparecer cuando pierda una vida
	private boolean spawning, visible;
	private Chronometer spawnTime, flickerTime;//titilar
	...
	public void update() 
	{
		...
		if(spawning) {
			...
			if(!flickerTime.isRunning()) {
				...
				flickerTime.run(Constants.FLICKER_TIME);//periodo de parpadeo
				visible = !visible;
		...
		//posicion del disparo y controlarlo
		if(KeyBoard.SHOOT &&  !fireRate.isRunning() && !spawning)
		{		
			gameState.getMovingObjects().add(0,new Shots(
	...
	//reaparecer posicion inicial del jugador, cada vez que "muera"
	...
	//para que las coliciones detecten si el jugador esta
